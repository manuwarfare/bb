...
func main() {
	...
	case "-n":
		if len(args) < 3 {
			fmt.Println("Error: Incorrect usage of -n. It should be: bb -n <name> <command>")
			return
		}
		name := args[1]
	...
	case "-r":
		if len(args) == 1 {
			fmt.Println("Error: Incorrect usage of -r. It should be: bb -r <name> [<name>...] or ./baby -r a")
			return
		}
		names := args[1:]
	...
	case "-c":
		if len(args) < 3 {
			fmt.Println("Error: Incorrect usage of -c. It should be: bb -c <name> <command>")
			return
		}
		name := args[1]
		command := strings.Join(args[2:], " ")
	...
	case "-ln":
		if len(args) != 2 {
			fmt.Println("Error: Incorrect usage of -ln. It should be: bb -ln <name>")
			return
		}
		name := args[1]
		showRule(name)
	case "-v":
		fmt.Println("BB version 1.0")
	case "-i":
        if len(args) != 2 {
            fmt.Println("Error: Incorrect usage of -i. It should be: bb -i <url or file path>")
            return
        }
        importSource := args[1]
	...
	default:
		if strings.HasPrefix(args[0], "-") {
			fmt.Println("Unrecognized option. Use bb -h to see the available options.")
		} else {
			runCommands(args)
		}
	}
...
func showHelp() {
	...
	fmt.Println("Create a new rule: bb -n update 'sudo apt update -y'")
	fmt.Println("The next time just run: 'bb update'")
	fmt.Println(" ")
	fmt.Println("For further help go to https://github.com/manuwarfare/bb")
...
func listRules() {
	...
	if err != nil {
		...
		fmt.Println("No rules have been created in BB yet.")
		return
	}
	defer file.Close()
...
func deleteAllRules() error {
    file, err := os.OpenFile("/etc/bb/bb.conf", os.O_WRONLY, 0644)
    if err != nil {
